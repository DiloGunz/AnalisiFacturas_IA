...
namespace ADIA.Repository.SqlServer.Generic;
...
public abstract class GenericRepository<T> where T : class
{
    ...
    {
        var eliminados = await _dbSet.Where(predicate).ExecuteDeleteAsync();
        return eliminados;
    }

    public virtual int ExecuteDelete(Expression<Func<T, bool>> predicate)
    {
        var numberEliminated = _dbSet.Where(predicate).ExecuteDelete();
    ...
    {
        var modificados = await _dbSet.Where(predicate).ExecuteUpdateAsync(updateFactory);
    ...
    {
        var modificados = _dbSet.Where(predicate).ExecuteUpdate(updateFactory);
        return modificados;
    }
    #endregion
